...
//	ulint		node_ptr_max_size = UNIV_PAGE_SIZE / 2;
	...
	btr_cur_t* cursor = new btr_cur_t; // ???
	page_cursor = btr_cur_get_page_cur(cursor);
	cursor->index = index;
	...
	int n_cnt_pio[2];
	page_id_t*	page_id_pio[2][128];
	buf_block_t* block_pio[2][128];
	ulint tree_savepoints_pio[2][128];
	int i,sv_pio=0;
	for (i=0;i<*pio_t;++i)
	{
		page_id_pio[0][i] = new page_id_t(dict_index_get_space(index),
					dict_index_get_page(index));
		page_id_pio[1][i] = new page_id_t(dict_index_get_space(index),
					dict_index_get_page(index));
	}

	n_cnt_pio[0] = 1;
	/*
	tree_savepoints_pio[0][0] = mtr_set_savepoint(mtr);
	block_pio[0][0] = buf_page_get_gen(page_id_pio[0][0],page_size,rw_latch,NULL,BUF_GET,file,line,mtr);
*/
	const page_size_t&	page_size = dict_table_page_size(index->table);
/*
	if (root_leaf_rw_latch == RW_X_LATCH) {
...
*/
	...
	for (;;) {
		...
		block_pio[sv_pio][0] = block;
		tree_savepoints_pio[sv_pio][0] = tree_savepoints[n_blocks];
...
printf("n_cnt_pio %d\n",n_cnt_pio[sv_pio]);
		if (height == 0 || n_cnt_pio[sv_pio] >= *pio_t) {
			if (rw_latch == RW_NO_LATCH
			    || srv_read_only_mode) {
				btr_cur_latch_leaves(
					block, page_id, page_size,
...
//removed
		...
		n_cnt_pio[1-sv_pio] = 0;
		ulint j;
int rec_cnt=0;
	int sum_rec=0;
for (i=0;i<n_cnt_pio[sv_pio];i++)
	sum_rec+=page_get_n_recs(buf_block_get_frame(block_pio[sv_pio][i]))-2;//???
	for (i=0;i<n_cnt_pio[sv_pio];i++)
		{
			ulint n_recs = page_get_n_recs(buf_block_get_frame(block_pio[sv_pio][i]));
	...
	printf("n_recs %d\n",n_recs);
			...
			{
				if (i == 0 && j == 0)
					block_pio[1-sv_pio][n_cnt_pio[1-sv_pio]] = block;
				else
				{
					if ((rec_cnt+1) * *pio_t < (n_cnt_pio[1-sv_pio]+1) * sum_rec)
					{
					page_cur_move_to_next(page_cursor_pio);
++rec_cnt;
	continue;				
			...
			offsets = rec_get_offsets(node_ptr, cursor->index, offsets,
				...
				if (offsets == NULL)
					printf("N\n");
			page_id_pio[1-sv_pio][n_cnt_pio[1-sv_pio]]->set_page_no(btr_node_ptr_get_child_page_no(node_ptr, offsets));
			tree_savepoints_pio[1-sv_pio][n_cnt_pio[1-sv_pio]] = mtr_set_savepoint(mtr);
			block_pio[1-sv_pio][n_cnt_pio[1-sv_pio]] = buf_page_get_gen(*page_id_pio[1-sv_pio][n_cnt_pio[1-sv_pio]],page_size,rw_latch,NULL,BUF_GET,file,line,mtr); 
				}
...
++rec_cnt;
	...
	for (i=0;i<n_cnt_pio[sv_pio];i++)
	{
		page_t* page = buf_block_get_frame(block_pio[sv_pio][i]);
		page_cur_t* page_cursor_pio = new page_cur_t;
		while(btr_page_get_level(page,mtr) > 0)
		{
		page_cur_set_before_first(block_pio[sv_pio][i],page_cursor_pio);
					page_cur_move_to_next(page_cursor_pio);
			node_ptr = page_cur_get_rec(page_cursor_pio);
			...
			page_id_pio[sv_pio][i]->set_page_no(btr_node_ptr_get_child_page_no(node_ptr, offsets));
					mtr_release_block_at_savepoint(
						mtr,
						tree_savepoints_pio[sv_pio][i],
						block_pio[sv_pio][i]);
